...
- **用户系统**：注册 / 登录 / 退出（Supabase Auth 模式下密码由 Supabase Auth 托管、token 为 Supabase access_token；未配置时回退自建 bcrypt + JWT；登录标识为**邮箱**，用户名作昵称）；未登录可浏览题库，练习/模拟考/错题本/统计需登录。
...
1. **注册/登录**：点击右上角"注册"创建账号（**邮箱** + 昵称（3~20 位字母数字下划线）+ 密码 ≥ 6 位），注册即登录；未登录也可浏览题库。
