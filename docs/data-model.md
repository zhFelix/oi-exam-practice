...
| 依据 | `docs/requirements.md` §7「数据字段要点」（PRD v1.0）+ 需求变更（竞赛类型扩展、题库扩充、LaTeX 公式示例） |
| 数据文件 | `data/questions.json`（种子题库 26 题）、`data/questions-extra.json`（其他竞赛扩展样例题 39 题） |
...
└── data/
    ...
    └── questions-extra.json  # 扩展题库（其他竞赛样例题，39 题：蓝桥杯/省市校级/其他 + LaTeX 示例）
```
...
### 内容格式约定（Markdown + LaTeX）

题干（`stem`）、选项文本（`options[].text`）、解析（`analysis`）与子题文本均支持以下两种标记语法，**两者可混用**：

1. **Markdown**：`**加粗**`、`*斜体*`、`` `行内代码` ``、``` ```代码块``` ```、`- 列表`、`[链接](url)` 等（`stem` 中如含程序可用代码块，如 reading 题另有独立 `code` 字段）。
2. **LaTeX 公式**：`$...$` 行内公式、`$$...$$` 块级公式；支持常见命令：`\binom{n}{k}`（组合数）、`\frac{a}{b}`（分数）、`\sqrt{x}`（根式）、`\log_2 n`、`\oplus`（异或）、`\times`、`^`/`_` 上下标等。
3. **约定**：
   - 公式一律用 `$` 包裹（含选项中的纯公式，如 `$O(n \log n)$`）；**普通文本中避免裸 `$` 字符**（如金额写作「5 元」而非 `5$`），防止误解析；
   - `$` 必须成对出现（行内/块级均如此）；数据校验脚本会检查文本中 `$` 数量为偶数；
   - 数据文件中 LaTeX 反斜杠按 JSON 规范转义为 `\\`（解析后为单个 `\`），存储与传输保持原样，**服务端不做公式求值，原样透传**；
   - 前端渲染顺序建议：HTML 转义 → Markdown 渲染（含代码块高亮）→ LaTeX 渲染；公式渲染失败时回退显示原始文本。
4. **示例题**：`q061`（递推公式 $T(n)=2T(n/2)+O(n)$）、`q062`（$\binom{5}{2}$）、`q063`（$\sqrt{2}$）、`q064`（异或 $a \oplus b$）、`q065`（幂/根式/分数/组合数综合多选）为 LaTeX 展示题，可直接用于前端公式渲染联调。
...
三批题目：第一批 q101~q113（13 题）+ 第二批 q040~q060（21 题，评审建议"题库 ≥ 60 题"扩充）+ 第三批 q061~q065（5 题，LaTeX 公式展示），共 **39 题**。
...
扩展题库题型：single 23、judge 9、multiple 5、reading 2（4 道子小题），全部含完整解析与难度；知识点分类（7 类）定义不变，对所有竞赛通用。主文件（26 题）+ 扩展（39 题）合并后总计 **65 题**（id 互不重叠：q001~q026、q040~q065、q101~q113，可直接合并导入）。
...
| v1.3 | 2025-01-04 | 新增 **LaTeX 公式展示题** q061~q065（5 题：递推式/组合数/根式/位运算/综合多选），题库增至 **65 题**；新增「内容格式约定（Markdown + LaTeX）」小节，扩展题库合计 39 题 |
